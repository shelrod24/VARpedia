...
if [ $IMAGE_COUNT -eq 1 ] ; then
	ffmpeg -loop 1 -y -t 1 -i image-000.jpg -vf "drawtext=fontfile=comic.ttf : fontsize=30 : fontcolor=white : x=(w-text_w)/2:y=(h-text_h)/2 : text='${1}',pad=ceil(iw/2)*2:ceil(ih/2)*2" "TEMPSHORT.mp4"
else
